...
class MaskLinear4bit(torch.nn.Module):
    ...
    def dequantize(self, x, scale):
        dequantized_x = x.to(torch.float32) * scale
        return dequantized_x
        
    def forward(self, x):
        #if torch.cuda.current_device() != x.device:
        #    torch.cuda.set_device(x.device)
        assert type(x) == quarot.PackedQuantizedTensor
        ...
        #     return self.dequantize(x.quantized_x, x.scales_x) + self.bias
        # else:
        #     return self.dequantize(x.quantized_x, x.scales_x)
        if self.bias is not None:
            return x.quantized_x + self.bias
        else:
            return x.quantized_x

    @staticmethod
    def from_float(module: torch.nn.Linear, weight_scales=None, mask=None):
        '''
        ...
        int_module = MaskLinear4bit(module.in_features, module.out_features, bias=module.bias is not None, dtype=weight_matrix.dtype).to(weight_matrix.dtype)
        if weight_scales is not None:
            ...
            if mask != None:
                mask = mask.cpu()
                int_module.weight = torch.index_select(int_module.weight, 0, mask)
                int_module.weight_scales = torch.index_select(int_module.weight_scales, 0, mask)
